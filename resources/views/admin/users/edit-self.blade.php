...
<div class="content-card">
    ...
    @if($isMainAdmin)
        <div class="alert alert-info">
            <i class="fas fa-crown me-2"></i> Anda login sebagai <strong>Administrator Utama</strong>. Anda dapat mengubah role akun Anda sendiri dari halaman ini.
        </div>
    @else
    ...
    <form action="{{ route('admin.profile.update.self') }}" method="POST">
        ...
        <div class="row g-4">
            ...
            {{-- ROLE: Hanya tampilkan dropdown aktif jika yang login adalah administrator utama --}}
            <div class="col-md-6">
                @if($isMainAdmin)
                    ...
                    <select name="role" class="form-select @error('role') is-invalid @enderror" required>
                        @foreach($roles as $role)
                            @php
                                $currentRole = old('role', $user->roles->isNotEmpty() ? $user->roles->first()->name : '');
                            @endphp
                    ...
                    <small class="text-muted">Sebagai administrator utama, Anda dapat mengubah role akun Anda sendiri.</small>
                @else
