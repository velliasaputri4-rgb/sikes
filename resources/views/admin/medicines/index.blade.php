...
@section('content')
    <div class="content-card">
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div>
                <h5 class="fw-bold mb-1"><i class="fas fa-pills me-2 text-primary"></i>Data Obat UKS</h5>
        ...
        <form method="GET" action="{{ route('admin.medicines.index') }}" class="mb-4">
            <div class="input-group">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.medicines.index') }}" class="btn btn-outline-secondary">Reset</a>
                @endif
            </div>
        </form>
                        ...
                        <tr>
                            ...
                            <td class="text-end pe-3">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('admin.medicines.edit', $med->id) }}" 
                                       class="btn btn-outline-warning btn-sm d-inline-flex align-items-center justify-content-center" 
                                       style="width: 36px; height: 36px;" 
                                       title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.medicines.destroy', $med->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus obat ini?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-5 text-muted">
                                ...
                                <a href="{{ route('admin.medicines.create') }}" class="btn btn-sm btn-outline-primary mt-2">Tambah Obat Pertama</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
